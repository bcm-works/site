...
detect_skill_domain() {
    ...
    if [[ "$file" =~ /site.class ]] || [[ "$file" =~ /server ]] || [[ "$file" =~ /page.layout ]] || [[ "$file" =~ /lume ]]; then
        detected_skills="analytics-error-tracking"
    elif [[ "$file" =~ /posts/ ]] || [[ "$file" =~ /lume ]]; then
        detected_skills="blog-publishing"
    elif [[ "$file" =~ /lume ]] || [[ "$file" =~ /site.class ]] || [[ "$file" =~ /build ]] || [[ "$file" =~ /deno.jsonc ]] || [[ "$file" =~ /build.test ]]; then
        detected_skills="build-pipeline"
    elif [[ "$file" =~ /railway ]] || [[ "$file" =~ /Dockerfile/ ]] || [[ "$file" =~ /docker-build.test ]] || [[ "$file" =~ /docker ]]; then
        detected_skills="infra-deployment"
    elif [[ "$file" =~ /lume ]] || [[ "$file" =~ /page.layout ]] || [[ "$file" =~ /robots ]] || [[ "$file" =~ /sitemap ]]; then
        detected_skills="seo-syndication"
    elif [[ "$file" =~ /posts/ ]] || [[ "$file" =~ /tags ]] || [[ "$file" =~ /tags/ ]]; then
        detected_skills="tags-taxonomy"
    elif [[ "$file" =~ /server ]] || [[ "$file" =~ /site.class ]]; then
        detected_skills="web-server-routing"
    fi
