...
## Domain purpose
`deno task build` runs `mise run build` (`.mise/tasks/build`), which assembles an ephemeral `build/` dir, runs Lume via `src/lume.ts`, bundles CSS, and copies static assets into `public/`. `src/site.class.ts` (`Site`) is the single source of env/config loading for both the build and the runtime server.
...
- Run `bash -n .mise/tasks/build` before proposing changes to that script.
